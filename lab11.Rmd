...
-- Date range: 2020-01-21 to 2020-10-27
-- Range of cases: 1 to 923828
-- Rnage of deaths: 0 to 33199
...
-- There are negative values for `new_cases` and `new_deaths` which doesn't make sense. There are some high deaths and cases value from New Jersey, Texas, and Georgia, but it seems that's because of the lag from getting the test results.
...
- Explore the pattern between $x$ and $y$ using `geom_smooth()`
  ...
  -- There is no evidence showing that pop_density is a correlate of newdeathsper100k as the straight line is near flat.
...
- Create a line chart of the `naive_CFR` for all states over time using `plot_ly()`
  ...
  -- Arkansas and Wyoming had a little peak inthe early September but it seems there was no significant peak in September. It flattened out over time.
...
- Create one more line chart, for Texas only, which shows `new_cases` and `new_deaths` together in one plot. Hint: use `add_lines()`
  ...
  -- There is about 2 weeks of time delay between the peak of cases and the peak of deaths.
...
-- When comparing two CFR maps, CFR today has much lower CFRs all over the states as we can see the color of CFR today map is lighter than that of CFR May 1 2020.
